...
```
## Qt
The Qt project has a .pro file and CMakeLists.txt. Either option works for compiling the program.

it can also be cross-compiled from Linux to Windows with the following setup for Ubuntu 24.04. Install the required packages on [the requirements](https://mxe.cc/#requirements-debian) aswell.
```
...
export PATH=/WhereMxeIsInstalled/usr/bin:$PATH

cd [QtProjectLocation]
mkdir build
cd build
/WhereMxeIsInstalled/usr/x86_64-w64-mingw32.static/qt5/bin/qmake ../RoutesGenereren.pro
make
```
...
| test.txt       | Locations for the game uniquely generated by the QT app |                   |
| %d.txt    | Quiz questions                                            | x                 |
...
Currently every file, aside from poko.txt, can be send from the Qt program to the SD card in the hardware.
